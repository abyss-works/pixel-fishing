...
const GLYPHS: Record<GlyphId, { cells: Cell[]; holes?: Cell[] }> = {
  // 봉투 — 테두리 사각형에 접힌 덮개(V자) 두 줄
  letter: {
    cells: [[0, 2, 12, 1], [0, 9, 12, 1], [0, 3, 1, 6], [11, 3, 1, 6],
            [1, 3, 2, 1], [9, 3, 2, 1], [3, 4, 2, 1], [7, 4, 2, 1], [5, 5, 2, 1]],
  },
  // 열쇠 — 고리 + 자루 + 이빨 (계정 연동)
  key: {
    cells: [[1, 3, 4, 1], [1, 7, 4, 1], [0, 4, 1, 3], [5, 4, 1, 3],
            [6, 5, 6, 2], [9, 7, 1, 2], [11, 7, 1, 2]],
    holes: [[2, 4, 2, 3]],
  },
  // 쿠폰 — 한쪽이 뜯긴 표
  ticket: {
    cells: [[0, 3, 12, 1], [0, 8, 12, 1], [0, 4, 1, 4], [11, 4, 1, 4], [5, 5, 1, 2], [7, 5, 1, 2]],
  },
  // 내보내기 — 아래를 향한 화살표 + 바닥
  download: {
    cells: [[5, 1, 2, 5], [3, 6, 6, 1], [4, 7, 4, 1], [5, 8, 2, 1], [1, 10, 10, 1]],
  },
  // 불러오기 — 위를 향한 화살표 + 바닥
  upload: {
    cells: [[5, 3, 2, 5], [5, 1, 2, 1], [4, 2, 4, 1], [3, 3, 6, 1], [1, 10, 10, 1]],
  },
  // 나가기 — 문틀 + 밖으로 나가는 화살표
  exit: {
    cells: [[1, 1, 5, 1], [1, 10, 5, 1], [1, 1, 1, 10], [7, 5, 4, 2],
            [9, 3, 1, 2], [9, 7, 1, 2], [10, 4, 1, 4]],
  },
