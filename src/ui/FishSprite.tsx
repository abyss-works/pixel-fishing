...
interface FishSpriteProps {
  ...
  /** 어느 폼으로 그릴지 — 변이는 변이 색 (폼 추가 시 COLOR_OF 행 추가) */
  form?: FormId;
  /** false면 실루엣(색·디테일 숨김) */
  discovered?: boolean;
...
export default function FishSprite({
  fish, preset, form = 'normal', discovered = true, ariaLabel, className,
}: FishSpriteProps) {
  ...
  useEffect(() => {
    ...
    const color = discovered ? (form === 'variant' ? fish.variant.color : fish.color) : SILHOUETTE;
    drawFishSprite(ctx, w / 2, h / 2, fish.shape, color, scale, discovered);
  }, [fish, form, discovered, w, h, scale]);
  return (
    <canvas
